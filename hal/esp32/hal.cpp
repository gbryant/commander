...
int hal_uart_getchar(uint32_t timeout_ms) {
    uint8_t c;
    return uart_read_bytes(UART_NUM_0, &c, 1, pdMS_TO_TICKS(timeout_ms)) == 1 ? (int)c : -1;
}
void hal_uart_putchar(char c)     { uart_write_bytes(UART_NUM_0, &c, 1); }
void hal_uart_puts(const char *s) { uart_write_bytes(UART_NUM_0, s, strlen(s)); }
