...
# R4 main: modules are managed by `cmdr module` via the generated
# commander_modules.h, so commander_setup just calls the generated hook.
R4_MAIN_CPP_TEMPLATE = """\
#include "commander.h"
...
def _emit_module(name: str, opts: dict, target: str):
    """Return (includes, decls, registers) C++ fragments for one module."""
    if name == "system":
...
def generate_modules_file(target: str, modules: dict, out_path: Path) -> None:
    ...
    registers: list = []
    for name in order:
        inc, dec, reg = _emit_module(name, modules.get(name, {}), target)
        for i in inc:
...
def _regenerate(target: str, modules: dict) -> None:
    src = Path("src")
    if not src.is_dir():
        die("no src/ directory — run from your project root")
    generate_modules_file(target, modules, src / "commander_modules.h")
    print(f"regenerated src/commander_modules.h")
...
def scaffold_arduino(target: str, name: str, out_dir: Path) -> None:
    ...
    if is_r4:
        ssid, password = wifi_credentials()
        (out_dir / "secrets.h").write_text(render(SECRETS_H_TEMPLATE, ssid=ssid, password=password))
        (src_dir / "main.cpp").write_text(render(R4_MAIN_CPP_TEMPLATE, name=name))
